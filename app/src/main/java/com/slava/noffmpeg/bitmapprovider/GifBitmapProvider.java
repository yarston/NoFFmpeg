...
public class GifBitmapProvider implements BitmapProvider {
    ...
    @Override
    public Bitmap next() {
        Log.w(TAG, "next " + frameCount + " " + framePointer + " " + currentImage);
        //if (frameCount <= 0 || framePointer < 0 || currentImage == null) return null;
        ...
        if (frame.lct == null) {
            ...
            act = frame.lct;
            if (bgIndex == frame.transIndex) bgColor = 0;
        }
    ...
    protected void readLSD() {
        ...
        try {
            ...
            Log.w(TAG, "readLSD" + width + "x" + height);
